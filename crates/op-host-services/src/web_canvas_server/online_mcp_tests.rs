...
// The role/editing tiers and the two workspace gates they guard are
// `include!`d rather than moved into a `mod`, so every test keeps its
// exact path and its position in the registration order. The fragment is
// the original lines 413-893, byte-for-byte.
include!("online_mcp_tests_role_tiers.rs");
